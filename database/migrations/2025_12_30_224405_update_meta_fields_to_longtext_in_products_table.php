<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;
use Illuminate\Support\Facades\DB;

return new class extends Migration
{
    public function up(): void
    {
        DB::statement("ALTER TABLE products MODIFY meta_title LONGTEXT NULL");
        DB::statement("ALTER TABLE products MODIFY meta_keywords LONGTEXT NULL");
        DB::statement("ALTER TABLE products MODIFY meta_description LONGTEXT NULL");
    }

    public function down(): void
    {
        DB::statement("ALTER TABLE products MODIFY meta_title VARCHAR(255) NULL");
        DB::statement("ALTER TABLE products MODIFY meta_keywords VARCHAR(255) NULL");
        DB::statement("ALTER TABLE products MODIFY meta_description TEXT NULL");
    }
};
